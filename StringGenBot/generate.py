...
ask_ques = "**• ذا كنت تريد تنصب ميوزك اختار بُِِآيَٕرٍِّٓوُِجَُِٕرُٰٖامٍْٓ\n\n• واذا تريد تنصب تٍَلَٕثيٍُوَٰٖنٌٍْٕ فأختار تيرمڪس\n\n• يوجد استخرجات جلسات لي البَُِوٍِْتٌُٕات**"


buttons_ques = [
    [
        InlineKeyboardButton("بُِِآيَٕرٍِّٓوُِجَُِٕرُٰٖامٍْٓ 💎", callback_data="pyrogram"),
        InlineKeyboardButton("تٍَلَٕثيٍُوَٰٖنٌٍْٕ 💎", callback_data="telethon"),
    ],
    [
        InlineKeyboardButton("بُِِآيَٕرٍِّٓوُِجَُِٕرُٰٖامٍْٓ بَُِوٍِْتٌُٕ 💎", callback_data="pyrogram_bot"),
        InlineKeyboardButton("تٍَلَٕثيٍُوَٰٖنٌٍْٕ بَُِوٍِْتٌُٕ 💎", callback_data="telethon_bot"),
    ],
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if api_id_msg.text == "/skip":
        ...
        try:
            ...
            return
        api_hash_msg = await bot.ask(user_id, "» حَُسَٕنٓا ِقِٕمٓ بأرٌْسِٕاُلٍ اٌٖلٍٓـ API_HASH", filters=filters.text)
    ...
    if not is_bot:
        await msg.reply(" أٍنتٰٓظرٍٕ سِٓـوُٓفٌِ نَٰـٍٕرٍُسُّٰـَلٍٓ كْٖٓـوٌَٰٓدٍٓ لِْْٰحٌّسٍَُٔابٍٍّكِ بُِالًَٕتٌٰليُجٍُـرٓٔآمَٕ .")
    else:
    ...
    try:
        phone_code_msg = None
        if not is_bot:
            phone_code_msg = await bot.ask(user_id, "[ارسل الكود زي اللي في الصوره](https://telegra.ph/file/da1af082c6b754959ab47.jpg)» \nاذا كانت كلمة المرور او الكود  هي\n 12345 يرجى ارسالها بالشكل التالي 1 2 3 4 5 مع وجود مسـافـات بين الارقام اذا احتجت مساعدة @HH_7_T.", filters=filters.text, timeout=600)
    ...
    if not is_bot:
        ...
        try:
            ...
            try:
                two_step_msg = await bot.ask(user_id, " ارسٍَِْٖل اٌٍِلباٌٍْْسُْٰٓورٌٌدٍٓ الٍُْٰخآُصٍُٖٔ بَْٖحسٌٍٰٰابْٓٓك .", filters=filters.text, timeout=300)
            except TimeoutError:
